...
class WellfoundScraper(BaseScraper):
    BASE_URL = "https://wellfound.com/jobs?query={query}&loc=Remote"
    ...
    async def search(self, query: str, remote_only: bool = True) -> List[Job]:
        ...
        async with async_playwright() as p:
            ...
            await page.wait_for_selector('[data-test="job-card"]', timeout=10000)
            
            job_cards = await page.query_selector_all('[data-test="job-card"]')
            
            for card in job_cards[:20]:
                try:
                    title_elem = await card.query_selector('[data-test="job-card-title"]')
                    title = await title_elem.inner_text() if title_elem else "N/A"
                    
                    company_elem = await card.query_selector('[data-test="job-card-company-name"]')
                    ...
                    location_elem = await card.query_selector('[data-test="job-card-location"]')
                    location = await location_elem.inner_text() if location_elem else "Remote"
                    
                    salary_elem = await card.query_selector('[data-test="job-card-salary"]')
