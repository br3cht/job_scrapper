...
class WeWorkRemotelyScraper(BaseScraper):
    BASE_URL = "https://weworkremotely.com/remote-jobs/search?search={query}"
    ...
    async def search(self, query: str, remote_only: bool = True) -> List[Job]:
        ...
        async with async_playwright() as p:
            ...
            for card in job_cards[:20]:
                try:
                    title_elem = await card.query_selector('.job-link')
                    ...
                    location_elem = await card.query_selector('.region')
